...
class test_severity_using_main(unittest.TestCase):
    ...
    def test_entity_with_configuration_and_fixed(self):
        lActual = subprocess.check_output(["bin/vsg", "-f", sEntityFileName, "-c", sConfigFile, "--fix"]).decode("utf-8").split("\n")
    ...
    def test_architecture_with_configuration_and_fixed(self):
        lActual = subprocess.check_output(["bin/vsg", "-f", sArchitectureFileName, "-c", sConfigFile, "--fix"]).decode("utf-8").split("\n")
    ...
    def test_both_with_configuration_and_fixed(self):
        lActual = subprocess.check_output(["bin/vsg", "-f", sEntityFileName, sArchitectureFileName, "-c", sConfigFile, "--fix"]).decode("utf-8").split("\n")
